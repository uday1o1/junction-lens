...
def audit_identities(
    ...
    for object_type in ("lane_segment", "traffic_element", "area"):
        ...
        occurrences: dict[tuple[str, str], list[tuple[int, tuple[int, ...], FloatArray]]] = (
            defaultdict(list)
        )
        within_frame_counts: dict[tuple[str, int, str], int] = defaultdict(int)
        for frame, source_id, signature, centroid in records:
            identity = (frame.key.segment_id, source_id)
            occurrences[identity].append((frame.key.timestamp_ns, signature, centroid))
            within_frame_counts[(frame.key.segment_id, frame.key.timestamp_ns, source_id)] += 1
...
def audit_report(
    ...
    *,
    ...
    """Create one deterministic JSON-ready M0 label and identity report."""
    capacity = audit_capacities(frames, capacities, required_coverage=required_coverage)
    identities = audit_identities(frames, continuity_thresholds)
    return {
